...
class PageAPIHandlerMixin:
    @classmethod
    async def pages_response_fully_rendered(
        cls,
        pages: list[DAOPages],
        ...
        for rel in page_asset_rels:
            if rel.page_id and rel.asset_id:
                ...
                asset_with_url = AssetResponse(
                    **AssetsOverviewResponse.model_validate(asset).model_dump(),
                    signed_asset_url=(
        ...
        return [
            PagesFullResponse(
                **PagesOverviewResponse.model_validate(page).model_dump(),
...
class PageAPIHandler(RouteHandler, PageAPIHandlerMixin):
    def register_routes(self) -> None:
        self.router.add_api_route(
            "/api/page/{page_id}/edit_text",
            self.page_edit_text,
            methods=["POST"],
            response_model=PagesFullResponse,
        )
    ...
    async def page_edit_text(
        ...
        async with self.app.new_db_session() as db_session:
            ...
            return PagesFullResponse(
                **PagesOverviewResponse.model_validate(updated_page).model_dump()
            )
