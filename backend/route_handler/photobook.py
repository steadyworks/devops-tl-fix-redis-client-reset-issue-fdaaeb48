...
from .page import PageAPIHandler, PagesFullResponse
...
class PhotobookAPIHandlerMixin:
    @classmethod
    async def photobook_response_fully_rendered(
        cls,
        ...
        pages_response_full = await PageAPIHandler.pages_response_fully_rendered(
            pages, db_session, asset_manager
        )
        return PhotobooksFullResponse(
            **PhotobooksOverviewResponse.model_validate(photobook).model_dump(),
            pages=pages_response_full,
        )


class PhotobookAPIHandler(RouteHandler, PhotobookAPIHandlerMixin):
    ...
    async def get_photobook_by_id(
        ...
        async with self.app.new_db_session() as db_session:
            ...
            return await self.photobook_response_fully_rendered(
    ...
    async def photobook_edit_title(
        ...
        async with self.app.new_db_session() as db_session:
            ...
            return PhotobooksOverviewResponse(
                **DAOPhotobooks.model_validate(photobook).model_dump()
            )
